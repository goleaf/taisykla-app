<div class="min-h-screen bg-gradient-to-br from-slate-50 via-indigo-50 to-purple-50 py-8">
    <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden relative">
            <div wire:loading wire:target="nextStep, previousStep, goToStep" class="absolute inset-0 bg-white/60 z-30 flex items-center justify-center backdrop-blur-[1px]">
                <div class="flex flex-col items-center">
                    <div class="relative w-16 h-16">
                        <div class="absolute inset-0 border-4 border-indigo-100 rounded-full"></div>
                        <div class="absolute inset-0 border-4 border-indigo-600 rounded-full border-t-transparent animate-spin"></div>
                    </div>
                    <span class="mt-4 text-sm font-semibold text-indigo-600">Loading step...</span>
                </div>
            </div>
            <div class="p-6 sm:p-8">
                    ...
                    <div class="space-y-6">
                        ...
                        @if(!$isClient)
                            <div class="relative">
                                ...
                                <svg class="absolute left-3 top-3.5 w-5 h-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" wire:loading.remove wire:target="customerSearch"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg>
                                <svg class="absolute left-3 top-3.5 w-5 h-5 text-indigo-500 animate-spin" fill="none" viewBox="0 0 24 24" wire:loading wire:target="customerSearch">
                                    <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                                    <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                                </svg>
                            </div>

                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 max-h-96 overflow-y-auto relative">
                                <div wire:loading wire:target="customerSearch" class="absolute inset-0 bg-white/50 z-10 flex items-center justify-center rounded-xl">
                                    <span class="text-sm font-medium text-indigo-600">Searching...</span>
                                </div>
                ...
                @elseif($currentStep === 2)
                    <div class="space-y-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 max-h-[28rem] overflow-y-auto relative">
                            <div wire:loading wire:target="equipmentSearch, equipmentLocationFilter, equipmentStatusFilter" class="absolute inset-0 bg-white/50 z-10 flex items-center justify-center rounded-xl">
                                <div class="flex flex-col items-center">
                                    <svg class="w-8 h-8 text-purple-600 animate-spin mb-2" fill="none" viewBox="0 0 24 24">
                                        <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                                        <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                                    </svg>
                                    <span class="text-sm font-medium text-purple-600">Filtering...</span>
                                </div>
                            </div>
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <div class="flex flex-wrap gap-2 relative">
                                <div wire:loading wire:target="problemTemplate" class="absolute inset-0 bg-white/30 z-10 flex items-center justify-center">
                                        ...
                                        <div class="relative">
                                            <div class="w-20 h-20 rounded-lg bg-gray-100 flex items-center justify-center overflow-hidden">
                                                @if(str_starts_with($media->getMimeType(), 'image/'))
                                                    <img src="{{ $media->temporaryUrl() }}" class="w-full h-full object-cover" loading="lazy">
                                                @else
    ...
    <div x-data="{ show: false }" @show-confirmation.window="show = true" x-show="show" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
        <div class="bg-white rounded-2xl shadow-2xl max-w-md w-full p-6" @click.away="show = false">
            ...
            <div class="flex gap-4">
                ...
                <button type="button" wire:click="confirmSubmit" @click="if (!$wire.isSubmitting) show = false" class="flex-1 px-4 py-2 rounded-lg bg-green-600 text-white disabled:opacity-50" wire:loading.attr="disabled">
                    <span wire:loading.remove wire:target="confirmSubmit">Confirm</span>
                    <span wire:loading wire:target="confirmSubmit">Processing...</span>
                </button>
            </div>
        </div>
    </div>
