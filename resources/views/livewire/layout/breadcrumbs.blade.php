            ...
            if ($action === 'index' || $action === null) {
                 ...
                 if ($action === 'show') {
                     ...
                     if ($model instanceof \Illuminate\Database\Eloquent\Model) {
                         $label = $model->name ?? $model->title ?? $model->subject ?? "Details (#{$model->id})";
        ...
        @foreach ($breadcrumbs as $breadcrumb)
            <li class="inline-flex items-center">
                ...
                @if ($breadcrumb['route'] && !$loop->last)
                    ...
                    @endphp
                    <a href="{{ route($breadcrumb['route'], $routeParams) }}" wire:navigate class="inline-flex items-center text-sm font-medium text-gray-700 hover:text-indigo-600 transition-colors duration-200">
                        @if ($loop->first)
                ...
                @else
                    <span class="inline-flex items-center text-sm font-medium {{ $loop->last ? 'text-gray-500 cursor-default' : 'text-gray-700' }}">
                        @if ($loop->first)
                            <svg class="w-3 h-3 me-2.5" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 20">
                                <path d="m19.707 9.293-2-2-7-7a1 1 0 0 0-1.414 0l-7 7-2 2a1 1 0 0 0 1.414 1.414L2 10.414V18a2 2 0 0 0 2 2h3a1 1 0 0 0 1-1v-4a1 1 0 0 1 1-1h2a1 1 0 0 1 1 1v4a1 1 0 0 0 1 1h3a2 2 0 0 0 2-2v-7.586l.293.293a1 1 0 0 0 1.414-1.414Z"/>
                            </svg>
